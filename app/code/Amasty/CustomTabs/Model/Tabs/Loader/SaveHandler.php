...
class SaveHandler
{
     ...
     * @param int $storeId
     */
    public function execute($layout, $storeId)
    {
        $this->saveDefaultTabs($layout, $storeId);
    }
     ...
     * @param int $storeId
     */
    private function saveDefaultTabs($layout, $storeId)
    {
        $updateTabsIds = [];
        foreach ($layout->getChildNames(self::TABS_NAME_IN_LAYOUT) as $tabName) {
            ...
            $updateTabsIds[] = $this->saveTab($tabData, $storeId);
        }
     ...
     * @return int
     */
    private function saveTab($tabData, $storeId)
    {
        $tab = $this->loadTab($tabData[TabsInterface::NAME_IN_LAYOUT]);
        if (!$tab->getTabId()) {
            $tab->addData($tabData);
        }
        $tab->addStore($storeId);
