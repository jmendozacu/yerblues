 ...
 * Class ProductTab
 */
class ProductTab extends \Magento\Framework\View\Element\Template implements IdentityInterface
{
    ...
    public function toHtml()
    {
        ...
        if ($this->getTab()) {
            ...
            $html = $this->addProductBlocks($html);
        }
     ...
     * @return string
     */
    protected function fixReviewTabBug($html)
    {
            ...
            switch ($type) {
                case TabsInterface::RELATED_ENABLED:
                    ...
                    $html = preg_replace(
                        '@(data-mage-init=\')(.*?)(\')@s',
                        sprintf(
                            '$1{"Amasty_CustomTabs/js/related-products"
                            :{"relatedCheckbox":".am-custom-tab-%1$s .am-tab-related.checkbox",
                            "selectAllLink":".am-custom-tab-%1$s [role=\"select-all\"]"}}$3',
                            $this->getTab()->getTabId()
                        ),
                        $html
                    );
     ...
     */
    protected function getProductBlock($type)
    {
        $productBlock = null;
